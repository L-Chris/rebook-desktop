...
impl Widget for Label {
    fn ui(self, ui: &mut Ui) -> Response {
        ...
        let visible = ui.is_rect_visible(response.rect);
        ...
        if visible {
            ...
            && ui
                ...
                .lock()
                .has_selection_in_viewport(ui.ctx().viewport_id())
        {
            // `LabelSelectionState::on_end_pass` clears a cross-label selection when either
            // endpoint was not visited during the frame. A `ScrollArea` still lays out labels
            // outside its clip rect, so keep those labels registered while a selection exists.
            // The painter clip prevents any offscreen text or selection shapes from leaking.
            LabelSelectionState::label_text_selection(
                ui,
                &response,
                galley_pos,
