...
impl LabelSelectionState {
    ...
    pub(crate) fn has_selection_in_viewport(&self, viewport_id: crate::ViewportId) -> bool {
        self.states
            .get(&viewport_id)
...
mod tests {
    ...
    #[test]
    fn offscreen_labels_keep_both_selection_endpoints_alive() {
        let ctx = Context::default();
        ...
        ctx.plugin::<LabelSelectionState>()
            ...
            layer_id: layer_id.unwrap(),
            primary: cursor(primary_id),
        ...
        assert!(
            ctx.plugin::<LabelSelectionState>()
                .lock()
                .has_selection(),
            "laying out both endpoints outside the clip rect must preserve the selection"
        );
    }
}
